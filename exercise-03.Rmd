...
You can simply download the pre-compiled version (MrBayes-3.2.7-WIN.zip). You'll find the executables in the `bin` sub-directory.
Click on the executable to open MrBayes. It should look like the figure below. Close the program for now.
...
Alternatively, Linux users can use [Conda](https://docs.conda.io/projects/conda/en/latest/user-guide/install/linux.html). 
...
You'll also need to download the software [Tracer](http://beast.community/tracer) and [FigTree](https://github.com/rambaut/figtree/releases). Both require installing [Java](https://adoptium.net/en-GB/temurin/releases).
...
Next let's open MrBayes and execute the above commands. Click on the `mb` executable. 
...
```{bash windows, eval=FALSE}
Execute primates_MrBayes.nex
```
...
```{r mcmc, eval=FALSE}
mcmc ngen = 20000 samplefreq = 100 printfreq = 100 nchains = 1 filename = /Users/iq75aqot/Documents/MrBayes_exercise/primates_JC;
```
